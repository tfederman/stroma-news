...
def log_server_error():
    ...
    if new_error_count >= MAX_SERVER_ERROR_COUNT:
        log.warning(
            f"server is now flagged as struggling ({new_error_count},{MAX_SERVER_ERROR_COUNT})"
        )
...
def recent_error_count():
    ...
    r = redis.Redis()
    error_timestamps = r.lrange(
        SERVER_ERROR_TIMESTAMPS_KEY, 0, SERVER_ERROR_TIMESTAMPS_MAX_LENGTH + 1
    )
