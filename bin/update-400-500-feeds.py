...
if __name__=="__main__":
    ...
    always_error_feeds = (
        FeedFetch.select(FeedFetch.feed_id, fn.count(FeedFetch.id), fn.Max(FeedFetch.status))
    ...
    for error_feed_id, count, status in always_error_feeds:
        ...
        feed_to_update = (
            Feed.select().where(Feed.id == error_feed_id).where(Feed.active == True).first()
        )

        if feed_to_update:
            log.info(
                f"Setting feed #{feed_to_update.id} inactive for status {status} ({count}) ({feed_to_update.uri})"
            )
